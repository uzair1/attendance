...
app.post('/attendance', (req, res) => {
    ...
    if ( !req.body.employeeCode ) {
        res.status(400).send(`Employee Code is required `);
        return;
    }

    Employee.find(
        { _Code: req.body.employeeCode },
        (err, data) => {
            if (!err) {
                res.send(data)
            } else {
                 res.status(500).send("Incorrect Code or something went wrong")
                 return;
            }
        });
    ...
    newAttendance.save((err, saved) => {
        if (!err) {
            res.send("Attendance has been Marked !!!");
        } else {
            res.status(500).send("Something went wrong, Please try later");
        }
    })
